<template>
  <router-link :to="`/oils/${oil.id}`" class="card block p-4 hover:shadow-md transition-shadow active:scale-[0.98] duration-150">
    <div class="flex items-center gap-3">
      <div :class="[meta.bg, 'w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0']">
        <span :class="[meta.text, 'text-base font-medium']">{{ meta.char }}</span>
      </div>
      <div class="flex-1 min-w-0">
        ...
        <div class="text-xs text-stone-400 mt-1">{{ oil.category }} · {{ oil.origin }}</div>
      </div>
