<template>
  <div class="flex flex-col h-screen md:h-[calc(100vh-4rem)]">
    <div class="flex flex-col h-full max-w-3xl mx-auto w-full">
      <!-- Header -->
      <div class="px-5 pt-10 pb-4 bg-gradient-to-b from-stone-50 to-white border-b border-stone-100 flex items-end justify-between md:px-8">
        <div>
        ...
        <button @click="store.clear()"
          class="text-xs text-stone-400 px-3 py-1.5 border border-stone-200 rounded-full tracking-wide mb-1 hover:border-stone-400 transition-colors">
          清除
        </button>
      </div>

      <!-- Messages -->
      <div ref="scrollEl" class="flex-1 overflow-y-auto px-4 py-5 space-y-4 bg-stone-50/50 md:px-8">
            ...
            <div class="max-w-[85%] space-y-2">
              ...
              <div v-if="msg.recommendations?.length" class="space-y-2">
                <router-link v-for="rec in msg.recommendations" :key="rec.oil_id"
                  :to="`/oils/${rec.oil_id}`"
                  class="flex items-start justify-between gap-2 bg-white rounded-xl p-3 border border-stone-100 active:scale-[0.98] transition-transform block hover:border-stone-300">
                  <div class="flex-1">
      ...
      <div v-if="store.messages.length <= 1 && !store.isLoading"
        class="px-4 py-3 border-t border-stone-100 bg-white md:px-8">
        ...
        <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          <button v-for="s in store.suggestedSymptoms" :key="s"
            ...
            class="flex-shrink-0 text-xs px-3 py-1.5 border border-stone-200 text-stone-500 rounded-full tracking-wide hover:border-stone-400 transition-colors">
            {{ s }}
          </button>
        </div>
      </div>
      ...
      <!-- Input -->
      <div class="px-4 py-3 bg-white border-t border-stone-100 flex gap-2 items-end md:px-8">
        <textarea
          v-model="input"
