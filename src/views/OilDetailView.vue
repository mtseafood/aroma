<template>
  <div v-if="oil">
    ...
    <div class="relative pt-14 pb-10 px-4 text-center transition-colors duration-300"
      ...
      <!-- Category badge -->
      <div :class="[oilMeta.bg, 'w-20 h-20 mx-auto rounded-xl flex items-center justify-center mb-6 shadow-sm']">
        <span :class="[oilMeta.text, 'text-3xl font-light tracking-wide']">{{ oilMeta.char }}</span>
      </div>

      <h1 class="text-2xl font-serif font-light text-stone-800 tracking-wide">{{ oil.chinese_name }}</h1>
      <p class="text-stone-400 text-sm mt-1.5">{{ oil.english_name }}</p>
    ...
    <div class="px-4 py-5 space-y-4">
      ...
      <!-- Intensity -->
      <div v-if="oil.intensity" class="card p-5">
        <h3 class="text-[10px] tracking-widest text-stone-400 uppercase mb-3">香氣強度</h3>
        <div class="flex gap-1">
          <div v-for="i in 10" :key="i"
            :class="['h-2.5 flex-1 rounded-sm transition-colors', i <= oil.intensity ? 'bg-primary-300' : 'bg-stone-100']"/>
        </div>
        <p class="text-xs text-stone-400 mt-2.5">{{ oil.aroma_profile }}</p>
      </div>
      ...
      <div class="card p-5">
        ...
        <div class="space-y-2">
          <div v-for="method in oil.usage_methods" :key="method.method"
            class="bg-stone-50 rounded-lg p-3.5">
            ...
            <p v-if="expanded.has(method.method)" class="text-xs text-stone-500 mt-2.5 leading-relaxed">
              {{ method.instruction }}
            </p>
          </div>
        </div>
      ...
      <!-- Precautions -->
      <div v-if="oil.precautions" class="card p-5 border-l-2 border-amber-300">
        <div class="flex gap-3">
          ...
          <div>
            <h3 class="text-[10px] tracking-widest text-stone-400 uppercase mb-1.5">注意事項</h3>
      ...
      <div v-if="related.length" class="card p-5">
        <h3 class="text-[10px] tracking-widest text-stone-400 uppercase mb-5">調香搭配</h3>
        <div class="flex gap-3 overflow-x-auto no-scrollbar">
          <router-link v-for="r in related" :key="r.id" :to="`/oils/${r.id}`"
            class="flex-shrink-0 flex flex-col items-center gap-1.5">
            <div :class="[relatedMeta(r.category).bg, 'w-14 h-14 rounded-lg flex items-center justify-center']">
